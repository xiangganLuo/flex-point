...
public class PluginLifecycleTest {
    ...
    static class RecordingPlugin extends AbstractPlugin {
        ...
        RecordingPlugin(String id, int order, List<String> trace) {
            this.id = id;
            this.trace = trace;
    ...
    void state_transitions_from_created_to_started_then_destroyed() {
        ...
        pm.register(new RecordingPlugin("p", 0, trace));
    ...
    @Test
    void install_forward_and_stop_reverse_order() {
        DefaultPluginManager pm = newManager();
        List<String> trace = new ArrayList<>();
        // order 决定安装顺序：a(0) -> b(1) -> c(2)
        pm.register(new RecordingPlugin("c", 2, trace));
        pm.register(new RecordingPlugin("a", 0, trace));
        pm.register(new RecordingPlugin("b", 1, trace));
        pm.resolve();
        pm.installAll();
        pm.stopAll();
